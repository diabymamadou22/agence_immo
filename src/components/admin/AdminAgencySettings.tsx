...
import { updateAgencyConfig, setAgencyPreset, resetAgencyConfig, AGENCY_PRESETS } from '../../store/agencySlice';
...
export const AdminAgencySettings: React.FC = () => {
  ...
  const selectedPresetId = useAppSelector((state) => state.agency.selectedPresetId);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    dispatch(
      addToast({
        type: 'success',
        message: 'Paramètres de l\'agence enregistrés avec succès ! Le site et les documents reflètent vos nouvelles informations.',
      })
    );
  };

  const handleApplyPreset = (presetId: string) => {
    dispatch(setAgencyPreset(presetId));
    dispatch(
      addToast({
        type: 'info',
        message: `Profil d'agence "${AGENCY_PRESETS.find(p => p.id === presetId)?.label}" chargé avec succès !`,
      })
    );
  };

  const handleReset = () => {
    if (window.confirm('Voulez-vous réinitialiser les informations de l\'agence au profil par défaut "Mali Immo Prestige" ?')) {
      dispatch(resetAgencyConfig());
      dispatch(
        addToast({
          type: 'info',
          message: 'Paramètres d\'agence réinitialisés.',
        })
      );
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400">Configuration Multi-Agence & Juridique</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-extrabold font-heading text-white">
            Personnalisation de l'Agence Immobilière
          </h2>
          <p className="text-xs sm:text-sm text-slate-300 max-w-2xl">
            Personnalisez le nom de l'agence, votre logo, vos numéros WhatsApp/Orange Money, vos coordonnées bancaires et votre cachet officiel qui apparaîtront automatiquement sur le portail public et tous les documents légaux.
          </p>
        </div>
      ...
      {/* 1-Click Agency Preset Switcher (Ideal for Selling / Demos) */}
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 sm:p-6 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-extrabold text-sm sm:text-base text-slate-900 font-heading">
            Démonstration Vente : Changez de Profil d'Agence en 1 Clic
          </h3>
        </div>
        <p className="text-xs text-slate-600">
          Sélectionnez un modèle d'agence pré-configuré pour tester instantanément la compatibilité marque blanche sur tout le système :
        </p>
          ...
          {AGENCY_PRESETS.map((preset) => (
            <button
              ...
              className={`p-3.5 rounded-xl text-left transition-all border cursor-pointer ${
                agencyConfig.name === preset.config.name
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-black">{preset.config.name}</span>
                {agencyConfig.name === preset.config.name && (
                  <CheckCircle2 className="w-4 h-4 text-slate-950" />
                )}
              </div>
              <p className="text-[11px] opacity-80 truncate">{preset.city} • {preset.config.phone}</p>
            </button>
          ))}
        </div>
      </div>
