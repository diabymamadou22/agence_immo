...
export const firestoreSyncMiddleware: Middleware = (storeAPI) => (next) => (action: any) => {
  ...
  try {
    ...
    if (type === 'properties/addProperty' || type === 'properties/updateProperty') {
      ...
      if (updatedItem) {
        firestoreService.saveProperty(updatedItem).catch(console.error);
      }
    } else if (type === 'properties/deleteProperty') {
      const propertyId = action.payload;
      if (propertyId) {
        firestoreService.deleteProperty(propertyId).catch(console.error);
      }
    } else if (type === 'properties/updatePropertyStatus') {
      const state = storeAPI.getState();
      const item = state.properties.items.find((p: any) => p.id === action.payload.id);
      if (item) {
        firestoreService.saveProperty(item).catch(console.error);
      }
    }
    ...
    // 2. TENANTS
      ...
      if (updatedTenant) {
        firestoreService.saveTenant(updatedTenant).catch(console.error);
      }
    } else if (type === 'tenants/deleteTenant') {
      const tenantId = action.payload;
      if (tenantId) {
        firestoreService.deleteTenant(tenantId).catch(console.error);
      }
    } else if (type === 'tenants/recordRentPayment' || type === 'tenants/recordPaymentReceipt') {
      const state = storeAPI.getState();
      const receipt = state.tenants.receipts[0];
      if (receipt) {
        firestoreService.saveReceipt(receipt).catch(console.error);
      }
      const tenant = state.tenants.items.find((t: any) => t.id === action.payload.tenantId);
      if (tenant) {
        firestoreService.saveTenant(tenant).catch(console.error);
      }
    } else if (type === 'tenants/deleteReceipt') {
      const receiptId = action.payload;
      if (receiptId) {
        firestoreService.deleteReceipt(receiptId).catch(console.error);
      }
    }
    ...
    // 3. LEADS
      ...
      if (lead) {
        firestoreService.saveLead(lead).catch(console.error);
      }
    } else if (type === 'leads/updateLeadStatus') {
      firestoreService.updateLeadStatus(action.payload.id, action.payload.status, action.payload.notes).catch(console.error);
    } else if (type === 'leads/deleteLead') {
      firestoreService.deleteLead(action.payload).catch(console.error);
    }
    ...
    // 4. OWNERS
      ...
      if (owner) {
        firestoreService.saveOwner(owner).catch(console.error);
      }
    } else if (type === 'owners/deleteOwner') {
      firestoreService.deleteOwner(action.payload).catch(console.error);
    } else if (type === 'owners/recordPayout') {
      const state = storeAPI.getState();
      const payout = state.owners.payouts[0];
      if (payout) {
        firestoreService.savePayout(payout).catch(console.error);
      }
    } else if (type === 'owners/deletePayout') {
      firestoreService.deletePayout(action.payload).catch(console.error);
    }
    ...
    // 5. CONTRACTS
      ...
      if (contract) {
        firestoreService.saveContract(contract).catch(console.error);
      }
    } else if (type === 'contracts/deleteContract') {
      firestoreService.deleteContract(action.payload).catch(console.error);
    }
    ...
    // 6. FINANCIAL EXPENSES
      ...
      if (exp) {
        firestoreService.saveExpense(exp).catch(console.error);
      }
    } else if (type === 'financials/deleteExpense') {
      firestoreService.deleteExpense(action.payload).catch(console.error);
    }
    ...
    // 7. AGENCY CONFIG
      ...
      if (state.agency.config) {
        firestoreService.saveAgencyConfig(state.agency.config).catch(console.error);
      }
    }
  } catch (err) {
